...
public class OrderServiceImplTest {
    ...
    public void addNewOrderItemToNewOrder_validInput_addedNewOrder() {
        ...
        Order order = orderService.addToBasket(customerId, "Latte", Arrays.asList("Milk", "Chocolate Sauce"));

        Mockito.verify(orderRepository).save(orderCaptor.capture());
        Mockito.verify(orderItemRepository).save(orderItemCaptor.capture());

        Mockito.verify(orderRepository, times(1)).save(order);

        Assertions.assertThat(orderCaptor.getValue().getCustomerId()).isEqualTo("elham");
        Assertions.assertThat(orderCaptor.getValue().getStatusType()).isEqualTo(Order.StatusType.IN_PROGRESS);

        Assertions.assertThat(orderItemCaptor.getValue().getProduct().getName()).isEqualTo("Latte");
        Assertions.assertThat(orderItemCaptor.getValue().getToppings().size()).isEqualTo(2);
        Assertions.assertThat(orderItemCaptor.getValue().getAmount()).isEqualTo(12D);
    }
